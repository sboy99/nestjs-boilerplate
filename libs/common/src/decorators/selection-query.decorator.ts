...
type TSelection<T extends AbstractEntity<T>> = {
...
export const SelectionQuery = createParamDecorator(
  <T extends AbstractEntity<T>>(selection: TSelection<T>, ctx: ExecutionContext): TSelectionQuery<T> => {
    ...
    return selectedFields.reduce<FindOptionsSelect<T>>((acc, field) => {
