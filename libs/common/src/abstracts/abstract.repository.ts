import type {
  ...
  TCreateManyDocumentParams,
  TCreateUniqueDocumentParams,
  ...
  TLookupDocumentParams,
  TPaginatedResource,
  TUpsertDocumentsParams,
...
> {
  ...
  // CRUD operations
  public async create<TDoc extends TDocument>({
    document,
    options,
  }: TCreateDocumentParams<TDoc>): Promise<TDoc> {
    ...
    if (!createdDocument?.[0])
      throw new BadRequestException(
        `Unable to create document ${this.docName} `
      );
    // log the response
    this.logger.log(
      `Created new ${this.docName} document ${JSON.stringify(
        createdDocument?.[0]?.toObject()
      )}`
    );
    // return the document
  ...
  public async createMany<TDoc extends TDocument>({
    ...
    const createdDocuments = _createdDocuments.map((createdDocument) =>
      createdDocument.toObject()
    );
    // log the response
    this.logger.log(
      `Created bulk ${this.docName} documents ${JSON.stringify(
        createdDocuments
      )}`
    );
  ...
  public async createUnique<TDoc extends TDocument>({
    ...
    if (!!isDoc)
      throw new BadRequestException(`${this.docName} document already exists`);
    return this.create({
      document,
  ...
  public async list<TDoc extends TDocument>({
    ...
    const sortOrder = (apiQuery?.sort || []).reduce<Record<string, string>>(
      (acc, curr) => {
  ...
  public async findOne<TDoc extends TDocument>({
    ...
    if (!options?.isNotToThrow && !document) {
      this.logger.warn(
        `${
          this.docName
        } document was not found with filterQuery ${JSON.stringify(
          filterQuery
        )}`,
        filterQuery
      );
  ...
  public async upsert<TDoc extends TDocument>({
    filterQuery,
    document,
    queryOptions,
  }: TUpsertDocumentsParams<TDoc>) {
    const isDoc = await this.lookUp({ filterQuery, queryOptions });
    if (!!isDoc)
      return this.findOneAndUpdate({
        filterQuery,
        updatedDocument: document as any,
        queryOptions,
      });
  ...
  public async findOneAndUpdate<TDoc extends TDocument>({
    ...
    if (!options?.isNotToThrow && !document) {
      this.logger.warn(
        `${this.docName} Document was not found with filterQuery`,
        filterQuery
      );
  ...
  public async findOneAndDelete<TDoc extends TDocument>({
    ...
    return this.model
      .findOneAndDelete({ ...filterQuery, isDeleted: false }, queryOptions)
      .lean<TDoc>(true);
  }
  ...
  public async withTransaction<TDoc>(
    session: ClientSession,
    fn: WithTransactionCallback<TDoc>
  ) {
    try {
      const res = await session.withTransaction<TDoc>(fn as any);
      this.logger.log(
        `transaction ${session.id?.id.toString('hex')} committed successfully`
      );
      ...
      this.logger.warn(
        `transaction ${session.id?.id.toString('hex')} rollbacked unfortunately`
      );
